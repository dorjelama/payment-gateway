...
namespace payment_gateway_backend.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> ProcessPayment([FromBody] PaymentRequestDto request)
        {
            ...
            {
                Log.Warning("Invalid payment request received: {ValidationMessage}", validationMessage);
                return BadRequest(new { message = validationMessage });
            }

            Log.Information("Processing payment for {CustomerEmail}, Amount: {Amount} {Currency}",
                request.CustomerEmail, request.Amount, request.Currency);

            var transaction = await SimulatePaymentGateway(request);

            Log.Information("Payment {Status} for {CustomerEmail}, Transaction ID: {TransactionId}",
